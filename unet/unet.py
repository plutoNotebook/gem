...
class BeatGANsUNetModel(nn.Module):
    ...
    def save_pretrained(self, pretrained_path):
        ...
        th.save(self.state_dict(), os.path.join(pretrained_path, "model.pt"))

    @classmethod
    def from_pretrained(cls, pretrained_path):
        with open(os.path.join(pretrained_path, "config.json"), mode="r") as f:
            config_dict = json.load(f)
        config = BeatGANsUNetConfig(**config_dict)

        model = cls(config)

        state_dict = th.load(
            os.path.join(pretrained_path, "model.pt"), map_location=th.device("cpu")
        )
...
class BeatGANsEncoderModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_path):
        with open(os.path.join(pretrained_path, "config.json"), mode="r") as f:
            config_dict = json.load(f)
        config = BeatGANsEncoderConfig(**config_dict)
